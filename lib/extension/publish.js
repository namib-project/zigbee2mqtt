        ...
        for (let [key, value] of entries) {
            ...
            try {
                if (topic.type === 'set' && converter.convertSet) {
                    ...
                    let propertyToGet;
                    if (topic.attribute) {
                        propertyToGet = topic.attribute;
                    } else {
                        propertyToGet = key;
                    }
                    logger.debug(`Publishing '${topic.type}' '${propertyToGet}' to '${resolvedEntity.name}'`);
                    await converter.convertGet(actualTarget, propertyToGet, meta);
                } else {
